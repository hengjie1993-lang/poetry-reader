...
> 当前版本：**v20**（`APP_VERSION = '20'`，新增收藏功能，数据池 1500+ 首）
...
- `genre` 为体裁标签：`诗` / `词` / `曲` / `楚辞` / `诗经`。**v19 起已做体裁筛选栏**（顶部横向 chip，带数量，与搜索框叠加）。
- `favorites` 为收藏列表，存于浏览器 `localStorage`（key=`poetry-reader-favs`），离线可用、不依赖后端。收藏键是 `作者·标题`（非顺序 id），即使 `poems.json` 重建重排也不会指错诗。
...
| 版本 | 现象 | 根因 | 解法 / 教训 |
|---|---|---|---|
| **v19** | 首版 v18 构建里《诗经》20 首整批丢失（关雎/蒹葭/采薇全无），且首屏无体裁筛选入口 | ① `collect_shijing` 套用通用 `too_long(单行≤16字)`，而诗经是四言两句式、整章一行可达 ~41 字，全部被误杀；② `genre` 仅作数据元信息，UI 未暴露 | ① `too_long` 加 `max_line` 参数，诗经传 60（仅保留「章数≤12」门槛），其余体裁保持 16；② 顶部加体裁 chip 栏（全部/诗/词/曲/楚辞/诗经 + 数量），`view` 叠加体裁+搜索过滤。**诗经 content 有的按句有的按章存储，单行长度参差，过滤时勿用死阈值** |
| **v20** | 家长需求：把孩子正在学/常读的诗标星，方便复习 | 纯前端无后端，收藏必须存本地 | ① 卡片右上角 `fav-btn` 星标按钮，`toggleFav()` 增删 `favorites`（`作者·标题` 作键）；② 体裁栏首位是「★ 收藏」胶囊，`showFavOnly` 切换仅看收藏；③ `localStorage` 持久化，离线可用；④ `mounted` 时读取还原。**收藏键务必用 `作者·标题` 而非 id**，否则重建 poems.json 后收藏会指向错诗 |
...
- v18 数据已带 `genre`（诗/词/曲/楚辞/诗经）字段；v19 已加**体裁筛选栏**（顶部横向 chip：全部/诗/词/曲/楚辞/诗经，带各体裁数量），与搜索框叠加生效，切换自动回到首首。
- v20 已加**收藏功能**：卡片右上角 ☆ 标星，顶部「★ 收藏」胶囊仅看收藏，存 `localStorage` 离线可用。
